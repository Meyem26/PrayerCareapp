...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1) Transfer or delete groups this user created (created_by is ON DELETE RESTRICT).
    ...
    // 2) Keep surviving group prayers: reassign creator_id to current group owner
    //    (otherwise ON DELETE CASCADE would remove shared group prayers).
    ...
    for (const prayer of groupPrayers ?? []) {
      ...
      if (!group?.created_by || group.created_by === userId) {
        // Group already deleted or still points at deleting user — drop the prayer.
    ...
    // 3) Delete auth user — profiles and personal data cascade from auth.users / profiles.
